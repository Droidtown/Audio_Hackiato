...
def getResult(inputSTR, utterance, args, resultDICT):
    ...
    if utterance == "佯裝公安之後線":
        resultDICT["fakeid"] = "線人"

    if utterance == "佯裝圍觀群眾之[女子]":
        resultDICT["fakeid"] = "群眾"
    ...
    if utterance == "佯裝該名[外]務":
        resultDICT["fakeid"] = args[0] + "務"

    if utterance == "佯裝該名[外]務走向[丙○○]停車處":
        resultDICT["fakeid"] = args[0] + "外"
    ...
    if utterance == "佯裝電信局[人員]":
        resultDICT["fakeid"] = "電信局" + args[0]

    if utterance == "假冒[○○醫院]員工":
        resultDICT["fakeid"] = args[0] + "員工"
    ...
    if utterance == "假冒[上海市][楊浦]公安局[經濟]犯罪[調查科][張思偉][科長]":
        resultDICT["fakeid"] = args[-2] + args[-1]
    ...
    if utterance == "假冒[中國][電信]或[醫][保局人員]":
        resultDICT["fakeid"] = "".join([args[0], args[1]])
    ...
    if utterance == "假冒[管區]警員":
        resultDICT["fakeid"] = "警員"

    if utterance == "假冒[網路][賣家]及[銀行][行員]":
        resultDICT["fakeid"] = "".join(args[0:2])

    if utterance == "假冒[網路][賣家]客服[人員]":
        resultDICT["fakeid"] = "客服" + args[-1]
    ...
    if utterance == "假冒[調查局][人員]或[律師]等身分":
        resultDICT["fakeid"] = args[0] + args[1]
    ...
    if utterance == "假冒之專員等頭銜":
        resultDICT["fakeid"] = "專員"

    if utterance == "假冒代理人[李國鼎]":
        resultDICT["fakeid"] = "代理人"
    ...
    if utterance == "假冒松[富]隆公司之員工":
        resultDICT["fakeid"] = "公司員工"

    if utterance == "假冒檢察官或警察":
        resultDICT["fakeid"] = "檢察官"

    if utterance == "假冒為[中華電信][股份][有限]公司":
        resultDICT["fakeid"] = "".join(args) + "公司"
    ...
    if utterance == "假冒醫保局人員":
        resultDICT["fakeid"] = "醫保局人員"

    if utterance == "假冒里民[王能貴]":
        resultDICT["fakeid"] = "里民"
    ...
    if utterance == "偽裝[銀行][內][應][可]協助[順利]貸款":
        resultDICT["fakeid"] = args[0] + "內應"

    if utterance == "偽裝○○公司廠長":
        resultDICT["fakeid"] = "廠長"

    if utterance == "偽裝成○○公司":
        resultDICT["fakeid"] = "公司"
    ...
    if utterance == "冒充警察[人員]":
        resultDICT["fakeid"] = "警察" + args[0]

    if utterance == "冒充郵局[人員]":
        resultDICT["fakeid"] = "郵局" +  args[0]
