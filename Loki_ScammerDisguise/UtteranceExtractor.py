...
if __name__ == "__main__":
    ...
    key_tokenLIST = ["偽裝", "冒充", "喬裝", "假裝", "假冒", "佯裝", "裝扮"]
    utteranceLIST = []

    fileLIST = os.listdir("./臺灣高等法院臺南分院")
    for f in fileLIST:
        print("{}/{} >> Processing: {}".format(fileLIST.index(f), len(fileLIST), f))
        with open("./臺灣高等法院臺南分院/{}".format(f)) as j:
            jdict = json.loads(j.read())

        if "詐欺" in jdict["reason"]:
                ...
                for r in resultDICT["result_pos"]:
                    if len(r) == 1:  #標點符號
                        ...
                        if any(k in sentence for k in key_tokenLIST):
                            ...
                            with open("CollectedUtterance.json", "a", encoding='utf8') as j:
